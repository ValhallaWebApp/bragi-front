...
async addReview(review: any, artwork: any): Promise<void> {
  try {
     ...
     this.authService.getUser().subscribe(async(user:any)=>{
      ...
      if (!user && !review.email) {
        ...
        // Salva la recensione nel database
         this.db.object(`/reviews/${artwork.id}/reviews/${id}`).set(reviewData).then((ele)=>{
           console.log('Recensione aggiunta con successo.',ele);

         }).catch((error)=>{
           throw new Error('Devi essere loggato o fornire un\'email valida per inviare una recensione.');
    ...
    throw new Error('Errore durante l\'aggiunta della recensione. Dettagli: ' + error.message);
  }
}
 ...
 getReviewsByUser(userId: string): Observable<any[]> {
    ...
    .pipe(
      switchMap(actions => {
        ...
        actions.forEach(artwork => {
          ...
          if (artworkId) {
            this.db.list(`/reviews/${artworkId}/reviews`, ref => ref.orderByChild('userId').equalTo(userId))
              .valueChanges()
              .subscribe(reviews => {
  ...
  getReviewsByArtwork(artworkId: string): Observable<any[]> {
    return this.db
      ...
      .pipe(
        catchError((error) => {
          console.error('Errore durante il recupero delle recensioni dell\'opera:', error);
          return of([]); // Restituisce un Observable con un array vuoto in caso di errore
        })
      );
  }
  ...
  updateReview(artworkId: string, reviewId: string, updatedReview: any): Promise<void> {
    return this.db
